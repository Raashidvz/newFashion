...
if (!isset($_SESSION["STAFF"])) {
    ...
    echo "<script>
            ...
            window.location.href = '../customers/login.php'; // Redirect to login page
          </script>";
                ...
                if ($result && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<select name='status' required>";
                            if($row['STATUSES']=='PENDING'){
                                echo "<option value='PENDING'" . ($row['STATUSES'] == 'pending' ? " selected" : "") . ">Pending</option>
                            ...
                            }else if($row['STATUSES']=='IN-PROGRESS'){
                                echo "<option value='IN-PROGRESS'" . ($row['STATUSES'] == 'in-progress' ? " selected" : "") . ">In Progress</option>
                                <option value='COMPLETED'" . ($row['STATUSES'] == 'completed' ? " selected" : "") . ">Completed</option>";
                            }else{
                                echo "<option value='COMPLETED'" . ($row['STATUSES'] == 'completed' ? " selected" : "") . ">Completed</option>";
                            }
